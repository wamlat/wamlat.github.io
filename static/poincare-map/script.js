    ...
    options: {
        ...
        plugins: {
            ...
            tooltip: {
                ...
                callbacks: {
                    label: (ctx) => `x: ${ctx.parsed.x.toFixed(4)}  z: ${ctx.parsed.y.toFixed(4)}`
                }
            }
        },
        scales: {
            x: {
                title: { display: true, text: 'x', color: '#555', font: { style: 'italic', family: 'Georgia, serif' } },
                grid: { color: '#1a1a25' },
                ticks: { color: '#444', font: { family: "'SF Mono', monospace", size: 11 } },
                border: { color: '#1e1e2e' }
            },
            y: {
                title: { display: true, text: 'z', color: '#555', font: { style: 'italic', family: 'Georgia, serif' } },
                grid: { color: '#1a1a25' },
                ticks: { color: '#444', font: { family: "'SF Mono', monospace", size: 11 } },
                border: { color: '#1e1e2e' }
            }
...
async function updateChart() {
    ...
    try {
        ...
        if (diverged) {
            warning.textContent = 'Trajectory diverged — the system is unstable for these parameters. Showing partial results.';
        ...
        if (err.name !== 'AbortError') {
            warning.textContent = 'Failed to reach the server. Is the backend running?';
            warning.classList.remove('hidden');
